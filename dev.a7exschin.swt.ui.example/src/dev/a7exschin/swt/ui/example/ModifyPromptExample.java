...
public class ModifyPromptExample {

	private static Display display = Display.getDefault();
	private static Shell shell = new Shell(display, SWT.DIALOG_TRIM | SWT.TITLE
			|SWT.BORDER |SWT.APPLICATION_MODAL);
	private static ListComplexClass entryToModify;

	public static void showModifyEntryView() {
		...
		for (String string : entryToModify.getAssignedStrings()) {
	...
	public static void main(String[] args) {
		...
		Collections.addAll(options, "-DoptionsVMargs=abcdefg", "-DjavaVersion=17");
		ListComplexClass entry = new ListComplexClass("HoweverLongEntryName", "very/very/very/very/long/path", options);
		entryToModify = entry;
		showModifyEntryView();
	...
	private static void okButton(String text) {
